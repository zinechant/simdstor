...
ODIR=m5out
mkdir -p $ODIR
~/gem5/build/$ARCH/gem5.${OPT} ${DEBUGFLAGS} --debug-file=log -d $ODIR ~/gem5/configs/example/se.py --param 'system.cpu[:].isa[:].sve_vl_se = 8' $CLOCK $CPUMEM $CACHES $scratchpad $streambuffer $prefetch  -c './varsimd' -o '--gtest_filter=*.* ' > >(tee stdout) 2> >(tee stderr >&2)
